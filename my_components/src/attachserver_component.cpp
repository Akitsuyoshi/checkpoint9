...
void AttachServer::callback(const std::shared_ptr<GoToLoading::Request> request,
                            std::shared_ptr<GoToLoading::Response> response) {
  ...
  if (future1.wait_for(std::chrono::seconds(15)) != std::future_status::ready) {
    timer_tf_->cancel();
    response->complete = false;
  ...
  timer_final_ = create_wall_timer(
      ...
      mutually_exclusive_group_2_);
  if (future2.wait_for(std::chrono::seconds(15)) != std::future_status::ready) {
    timer_final_->cancel();
    response->complete = false;
    RCLCPP_INFO(get_logger(), "Service timedout");
    graceful_shutdown();
...
void AttachServer::callback(const Odometry::SharedPtr msg) {
  std::lock_guard<std::mutex> lck(mutex_);
  odom_msg_ = msg;
}
...
std::pair<float, float> AttachServer::get_xy(int idx,
...
void AttachServer::publish_tf(bool is_static) {
  std::unique_lock<std::mutex> lck(mutex_);
...
void AttachServer::final_approach() {
  std::unique_lock<std::mutex> lck(mutex_);
...
double AttachServer::get_yaw(const geometry_msgs::msg::Quaternion &q) const {
...
void AttachServer::move_robot(double error_distance, double error_yaw,
